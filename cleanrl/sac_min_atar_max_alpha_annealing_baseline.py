    ...
    for global_step in progress_bar:
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                alpha_used = (torch.as_tensor(avg_return_normalised, device=device) + alpha_eps) / entropy
                # CRITIC training
                with torch.no_grad():
                    ...
                    next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (min_qf_next_target) + bias
