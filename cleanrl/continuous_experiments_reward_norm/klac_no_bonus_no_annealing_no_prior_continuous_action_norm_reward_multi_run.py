...
class NormMujoco(RewardWrapper):

    def __init__(self, env, div=1):
        super().__init__(env)
        self.div = div
...
class Args:
    ...
    n_sparse_steps: int = 1
    reward_norm: float = 1000.0


def make_env(env_id, seed, idx, capture_video, run_name, reward_norm):
    def thunk():
        if capture_video and idx == 0:
            ...
            env = gym.make(env_id)
        env = NormMujoco(env, div=reward_norm)
...
def _sigma_from_p(p: float, r: float, d: int) -> float:
    """
    Solve σ in   [erf(r / (√2 σ))]^d = p   ⟹   σ = r / (√2 · erf⁻¹(p^{1/d}))
    """
    if not (0.0 < p < 1.0):
        raise ValueError("`p` must lie strictly between 0 and 1.")
    # numerical safety: clip into the open unit interval
    inner = float(np.clip(p ** (1.0 / d), 1e-7, 1 - 1e-7))
    erf_inv = torch.erfinv(torch.tensor(inner)).item()        # inverse error-function
    return r / (math.sqrt(2.0) * erf_inv)


def _gaussian_entropy(sigma: float, d: int) -> float:
    """Differential entropy of an isotropic d-dimensional Gaussian."""
    return d * 0.5 * (1.0 + math.log(2.0 * math.pi)) + d * math.log(sigma)
...
"""
    ...
    for run_idx in range(args.n_runs):
        ...
        envs = gym.vector.SyncVectorEnv(
            [make_env(args.env_id, args.seed + i, i, args.capture_video, run_name, reward_norm)
        ...
        envs.close()
    out_dir = f"/hri/rawstreams/project/klac_2026-01/NormRewardContinuous/{args.env_id}_{args.exp_name}_pickles"
