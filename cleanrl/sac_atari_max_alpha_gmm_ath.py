    ...
    for global_step in progress_bar:
        ...
        if "final_info" in infos:
            for info in infos["final_info"]:
                ...
                episode_lengths.append(episodic_length[0])
                returns_per_step.append(episodic_return[0] / episodic_length[0])
                writer.add_scalar("charts/number_of_steps", np.sum(episode_lengths), global_step)
                writer.add_scalar("charts/number_of_samples", len(returns_per_step), global_step)

                # Keep samples relative to buffer size in steps
                if np.abs(np.sum(episode_lengths) - args.buffer_size) < 0.05 * args.buffer_size:
                    while np.sum(episode_lengths) > args.buffer_size:
                        returns_per_step.pop(0)
                        episode_lengths.pop(0)

                    reward_range = np.max(returns_per_step) - np.min(returns_per_step)
                    if reward_range == 0.0:
                        avg_return = np.array(np.mean(returns_per_step))
                        writer.add_scalar("charts/episodic_return_avg", avg_return, global_step)
                    else:
                        ## Apply GMM
                        gm = GaussianMixture(3).fit(np.vstack(returns_per_step))
                        cluster_return = np.max(gm.means_)
                        if cluster_return > highest_return:
                            highest_return = cluster_return 
                        avg_return = highest_return

                        writer.add_scalar("charts/episodic_return_avg", avg_return, global_step)
                        writer.add_scalar("charts/sample_population_mean", np.mean(returns_per_step), global_step)
                        writer.add_scalar("charts/worst_gaussian_mean", np.min(gm.means_), global_step)
                        writer.add_scalar("charts/median_gaussian_mean", np.median(gm.means_), global_step)
                        writer.add_scalar("charts/best_gaussian_mean", np.max(gm.means_), global_step)
                        writer.add_scalar("charts/absolute_difference_clustered_method", np.abs(avg_return - np.mean(returns_per_step)), global_step)
                        writer.add_scalar("charts/applied_clustering", 1, global_step)
                else:
                    avg_return = np.array(np.mean(returns_per_step))
                    writer.add_scalar("charts/episodic_return_avg", avg_return, global_step)
                    writer.add_scalar("charts/applied_clustering", 0, global_step)

                if returns_per_step[-1] < lowest_return:
                    lowest_return = returns_per_step[-1]

                avg_return_normalised = avg_return - lowest_return
