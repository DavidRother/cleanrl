...
"""
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                next_state_actions, next_state_log_pi, next_state_action_probs = actor.get_action(data.next_observations)
                qf1_next_target, ent_vals1_target = qf1_target(data.next_observations, next_state_actions)
                qf2_next_target, ent_vals2_target = qf2_target(data.next_observations, next_state_actions)
                ...
                ent_target = (alpha * next_state_log_pi +
                              (1 - data.dones.flatten()) * args.gamma * exp_ent_next).detach()

            qf1_a_values, ent_vals1 = qf1(data.observations, data.actions)
            qf2_a_values, ent_vals2 = qf2(data.observations, data.actions)
            qf1_loss = F.mse_loss(qf1_a_values.view(-1), next_q_value)
            qf2_loss = F.mse_loss(qf2_a_values.view(-1), next_q_value)
            qf_loss = qf1_loss + qf2_loss

            ent1_pred = ent_vals1.view(-1)
            ent2_pred = ent_vals2.view(-1)

            ent1_loss = F.mse_loss(ent1_pred, ent_target)
            ent2_loss = F.mse_loss(ent2_pred, ent_target)
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    qf1_pi, ent1 = qf1(data.observations, pi)
                    qf2_pi, ent2 = qf2(data.observations, pi)
                    min_qf_pi = torch.min(qf1_pi, qf2_pi)
                    min_ent = torch.min(ent1, ent2)
                    actor_loss = ((alpha * log_pi) - (min_qf_pi - alpha * min_ent)).mean()
