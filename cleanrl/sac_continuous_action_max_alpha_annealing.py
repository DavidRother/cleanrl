    ...
    for global_step in progress_bar:
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha_used * next_state_log_pi
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    actor_loss = ((alpha_used * log_pi) - min_qf_pi).mean()
            ...
            if global_step % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    std = log_std.exp()
                    normal = torch.distributions.Normal(mean, std)
                    x_t_sample = normal.rsample()
                    joint_log_density = normal.log_prob(mean).sum(dim=-1)
                    avg_limiting_density = torch.exp(joint_log_density).mean().item()
                writer.add_scalar("charts/limiting_density", avg_limiting_density, global_step)
                if args.autotune:
