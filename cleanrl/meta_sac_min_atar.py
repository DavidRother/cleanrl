...
class Args:
    ...
    target_entropy_min: float = 0.0
    target_entropy_max: float = 1.7
    curvature_ema_beta: float = 0.001
    curvature_meta_lr: float = 1e-3
    entropy_meta_lr: float = 1e-4
    ...
    for global_step in progress_bar:
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                # per‐param average (or remove `/flat.numel()` for full trace)
                C_cur = (flat.pow(2).sum() / flat.numel()).item()

                # 2) estimate current policy entropy E_cur
                #    log_pi2 was [B,1] per-sample log‐prob from the fresh forward
                E_cur = (-log_pi).mean().item()

                # 3) update EMA of curvature
                C_target = (1 - args.curvature_ema_beta) * C_target + args.curvature_ema_beta * C_cur

                rel_err = (C_cur - C_target) / (C_target + 1e-8)

                # 2) proportional entropy error
                ent_err = (E_cur - H) / (abs(H) + 1e-8)

                # 4) combined meta‐step on target_entropy H
                delta_H = (
                        - args.curvature_meta_lr * rel_err
                        - args.entropy_meta_lr * ent_err
                        - args.reg_lambda * (H - basin_entropy)
                )
                H = float(np.clip(H + delta_H, args.target_entropy_min, args.target_entropy_max))

                # 5) write it back into SAC’s dual‐tune
                target_entropy = H
                ...
                writer.add_scalar("meta/Policy_entropy", E_cur, global_step)
                writer.add_scalar("meta/target_entropy", H, global_step)
                writer.add_scalar("meta/ent_err", ent_err, global_step)
                writer.add_scalar("meta/rel_err", rel_err, global_step)
