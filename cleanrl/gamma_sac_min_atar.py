...
"""
    ...
    writer = SummaryWriter(f"runs_gamma/{run_name}")
    writer.add_text(
        "hyperparameters",
    ...
    log_gamma = torch.ones(1, requires_grad=True, device=device)
    gamma = log_gamma.exp().item()
    gamma_optimizer = optim.Adam([log_gamma], lr=args.q_lr, eps=1e-4)
    ...
    for global_step in progress_bar:
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                gamma_loss = log_gamma.exp() * (data.rewards.mean() + log_pi.mean()).detach()
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("losses/gamma", gamma, global_step)
                ...
                writer.add_scalar("losses/gamma_loss", gamma_loss.item(), global_step)
                writer.add_scalar("losses/entropy_factor", rews / ent, global_step)
