...
"""
    ...
    for run_idx in range(5):
        ...
        envs = gym.vector.SyncVectorEnv(
            [make_env(args.env_id, current_seed + i, i, args.capture_video, run_name) for i in range(args.num_envs)]
        )
        ...
        for global_step in progress_bar:
            ...
            if global_step > args.learning_starts:
                if global_step % args.train_frequency == 0:
                    delta_t = min(delta_end, delta_start + (delta_end - delta_start) *
                    ...
                    with torch.no_grad():
                        ...
                        v_next = torch.sum(action_probs * next_q, dim=1)
                    ...
                    # --- build logits whose gradient is frozen only on the taken action
                    #     (same numerical value, but detached from the graph)
                    q_for_kl = q_pred.clone()  # (B, A) keeps grad
                    taken_logits_detached = q_pred.gather(1, data.actions).detach()
                    q_for_kl.scatter_(1, data.actions,
                                      taken_logits_detached)  # overwrite just that column with detached tensor

                    # --- KL(p || uniform) over ALL actions
                    probs = F.softmax(q_for_kl / temperature, dim=1)  # (B, A)
                    logA = math.log(A)
                    entropy = -(probs * (probs + 1e-12).log()).sum(dim=1)  # (B,)
                    kl_batch = logA - entropy  # (B,)

                    # --- hinge
                    violation = kl_batch - delta_t
                    hinge_mean = violation.mean()

                    primal_loss = td_loss + 0.01 * hinge_mean
                    ...
                    with torch.no_grad():
                        qf_values = q_network(data.observations)
                    ...
                    entropy_per_state = -(action_probs * log_pi).sum(dim=1)  # [B]
                    kl_per_state = math.log(num_actions) - entropy_per_state  # [B]

                    # dual update for α:
                    kl_detached = kl_per_state.detach()
                    alpha_loss = -(log_alpha.exp() * (kl_detached - delta_t)).mean()
                    ...
                    if (global_step % (args.train_frequency * 25)) == 0:
                        writer.add_scalar(f"{run_prefix}/losses/td_loss", td_loss, global_step)
                        writer.add_scalar(f"{run_prefix}/losses/q_values", q_taken.mean().item(), global_step)
                        sps = int(global_step / (time.time() - start_time))
                        writer.add_scalar(f"{run_prefix}/charts/SPS", sps, global_step)
                        writer.add_scalar(f"{run_prefix}/charts/entropy", entropy_per_state.mean().item(), global_step)
                        writer.add_scalar(f"{run_prefix}/charts/delta", delta_t, global_step)
                        writer.add_scalar(f"{run_prefix}/charts/kl_mean", kl_detached.mean().item(), global_step)
                        writer.add_scalar(f"{run_prefix}/charts/kl_q_network_mean", kl_batch.mean().item(), global_step)
                        writer.add_scalar(f"{run_prefix}/charts/alpha", alpha, global_step)
