...
def project_pi(pi_old):
    low, high = 0.0, 1.0
    for _ in range(20):
        mid = 0.5*(low + high)
        p_mid = pi_old.pow(mid)
        p_mid = p_mid / p_mid.sum()
        D_mid = (p_mid * torch.log(p_mid * num_actions)).sum()
        if D_mid > delta:
            low = mid
        else:
            high = mid
    p_alpha = pi_old.pow(high)
    ...
    for global_step in progress_bar:
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                q1_all = qf1(data.observations)  # [B, A]
                q2_all = qf2(data.observations)  # [B, A]
                batch_size, num_actions = q1_all.shape


                # build projection‐targets
                with torch.no_grad():
                    proj_targets = torch.empty_like(q1_all)  # we'll share the same target for both nets
                    for i in range(batch_size):
                        # use the *post-Bellman* Q-values as "old"
                        q_old = 0.5 * (q1_all[i] + q2_all[i])
                        pi_old = torch.softmax(q_old / tau_2, dim=-1)
                        D_old = (pi_old * torch.log(pi_old * num_actions)).sum()
                        if D_old <= delta:
                            # within the KL‐ball: keep Q exactly as is
                            proj_targets[i] = q_old
                        else:
                            # project
                            pi_proj = project_pi(pi_old)
                            a_star = data.actions.long()[i]
                            C = q_old[a_star] - tau_2 * torch.log(pi_proj[a_star])
                            proj_targets[i] = tau_2 * torch.log(pi_proj) + C

                proj_loss = F.mse_loss(q1_all, proj_targets) + F.mse_loss(q2_all, proj_targets)
