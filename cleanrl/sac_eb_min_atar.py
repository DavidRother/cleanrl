...
class Args:
    ...
    target_entropy_min_scale: float = 0.8
    """multiplier for log(|A|) that gives the *lower* entropy bound H_min"""
    target_entropy_max_scale: float = 1.2
...
"""
    ...
    log_num_actions = torch.log(torch.tensor(float(n_actions)))

    # lower & upper entropy targets (in nats)
    H_min = args.target_entropy_min_scale * log_num_actions
    H_max = args.target_entropy_max_scale * log_num_actions
    ...
    for global_step in progress_bar:
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                lambda_L = torch.clamp(lambda_L + args.lambda_lr * (H_min.item() - entropy_batch), min=0.0)
                lambda_U = torch.clamp(lambda_U + args.lambda_lr * (entropy_batch - H_max.item()), min=0.0)
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("charts/lambda_L", lambda_L.item(), global_step)
                writer.add_scalar("charts/lambda_U", lambda_U.item(), global_step)
