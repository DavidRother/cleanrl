        ...
        for global_step in progress_bar:
            ...
            if "final_info" in infos:
                for info in infos["final_info"]:
                    ...
                    writer.add_scalar(f"{run_prefix}/charts/alpha_upper_bound", avg_return_normalised, global_step)
                    break
            ...
            if global_step > args.learning_starts:
                if global_step % args.update_frequency == 0:
                    ...
                    alpha_used = min(alpha,
                                     (torch.as_tensor(avg_return_normalised, device=device) + args.alpha_eps) / entropy)
                    # CRITIC training
                    with torch.no_grad():
