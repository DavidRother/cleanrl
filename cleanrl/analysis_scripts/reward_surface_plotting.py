...
# Reward Evaluation
# -------------------------------
def evaluate_reward(model, env, num_episodes=5):
    """
...
def main():
    ...
    chosen_model = "final_model_seed_123456.pt"

    model_path = f"../runs/MinAtar/SpaceInvaders-v1__sac_min_atar_max_alpha_multi_run/{chosen_model}"
    ...
    save_filename = f"reward_surface_data_{chosen_model}.npz"
    save_path = os.path.join(model_dir, save_filename)
    np.savez(save_path, X=X, Y=Y, reward_grid=reward_grid)
    print(f"Reward surface data saved at: {save_path}")

    plot_filename = f"reward_surface_plot_{chosen_model}.png"
