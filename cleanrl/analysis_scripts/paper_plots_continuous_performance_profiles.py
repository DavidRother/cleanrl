...
def normalise_to_sac(score_dict: dict, sac_key: str = "SAC", ref_func=np.mean) -> dict:
    """Divide every run by the SAC reference (per task)."""
    if sac_key not in score_dict:
        raise KeyError(f"SAC baseline '{sac_key}' not found in score_dict")

    sac_scores = score_dict[sac_key]            # shape (runs, tasks)
    sac_reference = ref_func(sac_scores, axis=0)  # (tasks,)

    sac_reference = np.where(sac_reference == 0, 1e-8, sac_reference)  # avoid /0

    norm_dict = {algo: arr / sac_reference for algo, arr in score_dict.items()}
    return norm_dict
...
if __name__ == "__main__":
    ...
    # ❶ Determine a sensible τ range from the data
    ...
    fig_pp.savefig(OUTDIR / 'mujoco_perf_profile.svg', format='svg', bbox_inches='tight')
    fig_pp.savefig(OUTDIR / 'mujoco_perf_profile.png', format='png', dpi=300, bbox_inches='tight')
