    ...
    for global_step in progress_bar:
        ...
        if "final_info" in infos:
            for info in infos["final_info"]:
                ...
                if len(returns_per_step) > 50:
                    returns_per_step.pop(0)

                    clustering = DBSCAN(eps=(np.max(returns_per_step) - np.min(returns_per_step)) * 0.05, min_samples=int(len(returns_per_step) * 0.05)).fit(np.vstack(returns_per_step))
